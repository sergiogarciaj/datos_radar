...
DECLARE target_table STRING DEFAULT 'cus-data-dev.radar2.sgj_third';
SET start_date = DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), MONTH);

CREATE TEMP TABLE sgj_third_temp AS
WITH
...
basefinal AS (
  SELECT
    * EXCEPT(category, kept_bot_category),
    CASE category
  ...
  FROM (
    SELECT
      ...
      CASE
        WHEN tp.canal = 'cases' THEN (cases_cat.first_category IS NULL AND tp.cat_pca IS NULL AND (tp.cat_bot IS NOT NULL OR tip_data.tip IS NOT NULL))
        WHEN tp.canal = 'voz'   THEN (tp.cat_pca IS NULL AND (tp.cat_bot IS NOT NULL OR tip_data.tip IS NOT NULL))
        ELSE FALSE
      END AS kept_bot_category
...
pro AS (
  SELECT
    ...
    canal,
    market_code,
    SUM(CASE WHEN category <> 'NOT_CATEGORIZED' AND kept_bot_category = FALSE THEN factor ELSE 0 END) AS fac_cat,
...
with_factor_plus AS (
  SELECT
    ...
    CASE
      ...
      -- Si es tipificación de bot no pisada por PCA para HUMAN -> 0
      WHEN fb.kept_bot_category = TRUE AND fb.is_human = 'HUMAN' THEN 0
...
EXECUTE IMMEDIATE FORMAT("DELETE FROM `%s` WHERE call_date >= @start_date", target_table) USING start_date AS start_date;

EXECUTE IMMEDIATE FORMAT("INSERT INTO `%s` SELECT * FROM sgj_third_temp", target_table);

DROP TABLE sgj_third_temp;
